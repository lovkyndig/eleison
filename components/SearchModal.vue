      ...
      if (pagefind) {
        try {
          ...
          if (metaResults.results.length > 0) {
            ...
            if (appConfig?.basic?.search?.exclude && appConfig.basic.search.exclude.length > 0) {
              filterResults = resultsData.filter(item => !appConfig.basic.search.exclude.includes(item.url))
            }
