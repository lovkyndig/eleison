...
export default defineEventHandler(async (event) => {
  ...
  const signedSession = sign(session, config.auth.cookieSecret)

  setCookie(event, config.auth.cookieName, signedSession, {
    httpOnly: true,
    path: '/',
    sameSite: 'strict',
    secure: process.env.NODE_ENV === 'production',
    expires: rememberMe ? new Date(Date.now() + config.auth.cookieRememberMeExpires) : new Date(Date.now() + config.auth.cookieExpires),
  })
